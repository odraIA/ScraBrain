...
% 4 ---------------------------------------------------------------------------
\begin{frame}[c]{Invasivo vs. no invasivo}
  \small
  \begin{columns}[c,totalwidth=\linewidth]
    \begin{column}{0.34\linewidth}
      \centering
      \includegraphics[width=\linewidth,height=0.29\textheight,keepaspectratio]{ecog.jpg}\par
      \vspace{0.5mm}
      {\bfseries ECoG}\par
      \vspace{3mm}
      \includegraphics[width=\linewidth,height=0.31\textheight,keepaspectratio]{imagenes/eeg.png}\par
      \vspace{0.5mm}
      {\bfseries EEG}
    \end{column}
    ...
    \begin{column}{0.31\linewidth}
      \centering
      \includegraphics[width=\linewidth,height=0.70\textheight,keepaspectratio]{MEG.jpg}\par
      \vspace{1mm}
      {\bfseries MEG}
    \end{column}
    ...
    \begin{column}{0.29\linewidth}
      \centering
      \includegraphics[width=\linewidth,height=0.29\textheight,keepaspectratio]{fmri.jpg}\par
      \vspace{0.5mm}
      {\bfseries fMRI}\par
      \vspace{3mm}
      \includegraphics[width=\linewidth,height=0.29\textheight,keepaspectratio]{imagenes/perrikaryal.png}\par
      \vspace{0.5mm}
      {\bfseries BCI con EEG en videojuegos}
    \end{column}
  \end{columns}

  \note{Usar esta diapositiva para aterrizar visualmente las siglas. ECoG queda en el lado invasivo; EEG, MEG y fMRI son no invasivos; el ejemplo de Perrikaryal ayuda a conectar EEG con BCI real o divulgativa antes de pasar a la comparación técnica.}
\end{frame}
  ...
  \begin{columns}[c,totalwidth=\linewidth]
    ...
    \begin{column}{0.59\linewidth}
      ...
      \begin{exampleblock}{Resumen}
        Registra señales de microvoltios mezcladas con  mucho ruido. Precisamente por eso hacen falta preprocesamiento, contexto largo y preentrenamiento.
      \end{exampleblock}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T,totalwidth=\linewidth]
    ...
    \begin{column}{0.36\linewidth}
      \begin{block}{Idea central}
        Modelo de pocos parámetros que sea capaz de entender MEG y especializar con pocos datos.
      \end{block}
  ...
  \begin{columns}[c,totalwidth=\linewidth]
    \begin{column}{0.31\linewidth}
      \begin{block}{Lectura}
        ...
        \begin{itemize}
          \item ZuCo 2.0: 11,1 h NR; lectura natural.
          \item Nieuwland et al.: $\sim$170 h; lectura palabra a palabra.
    ...
    \begin{column}{0.31\linewidth}
      \begin{block}{Escucha}
        \begin{minipage}[t][29mm][t]{\linewidth}
        \begin{itemize}
          \item SparrKULee: 168 h; habla natural.
          \item OpenNeuro ds007808: 33,8 h escucha + 167 h contexto.
    ...
    \begin{column}{0.31\linewidth}
      \begin{alertblock}{Evaluación final}
        \begin{minipage}[t][29mm][t]{\linewidth}
        \begin{itemize}
          \item OpenNeuro ds004408: 20,6 h.
  ...
  \begin{columns}[c,totalwidth=\linewidth]
    \begin{column}{0.37\linewidth}
      ...
      \begin{alertblock}{Importante}
        \small
        Nyquist fija la frecuencia máxima tras el remuestreo:
        \[
          f_N = f_s/2 = 50/2 = 25\,\text{Hz}
        \]
        Aunque el filtro llegue a 40 Hz, el modelo usa aprox. 0,1--25 Hz.
      \end{alertblock}
      \end{minipage}
    \end{column}
    ...
    \begin{column}{0.59\linewidth}
      ...
      \begin{tikzpicture}[node distance=3mm, every node/.style={font=\tiny}]
        \node[draw=greenish, fill=greenish!10, rounded corners, text width=26mm, align=center] (sigfreq)
          {\textbf{frecuencia EEG}\\oscilaciones de la señal\\0,1--40 Hz};
        \node[draw=warning, fill=warning!16, rounded corners, text width=26mm, align=center, right=of sigfreq] (samplefreq)
          {\textbf{frecuencia de muestreo}\\muestras por segundo\\50 Hz};
      \end{tikzpicture}

      \vspace{2mm}

      \begin{tikzpicture}[node distance=3.4mm, every node/.style={font=\scriptsize}]
        \node[draw=deepgreen, fill=deepgreen!8, rounded corners, text width=40mm, align=center] (raw) {EEG heterogéneo};
        \node[draw=greenish, fill=greenish!12, rounded corners, text width=40mm, align=center, below=of raw] (filter) {filtro 0,1--40 Hz};
        \node[draw=warning, fill=warning!18, rounded corners, text width=40mm, align=center, below=of filter] (resample) {remuestreo a 50 Hz};
        \node[draw=accent, fill=accent!12, rounded corners, text width=40mm, align=center, below=of resample] (win) {ventanas completas de 150 s};
        \node[draw=deepgreen, fill=deepgreen!8, rounded corners, text width=40mm, align=center, below=of win] (mask) {posiciones 3D + máscaras};
      ...
      \begin{tikzpicture}[
        ...
        fullwin/.style={draw=deepgreen, fill=deepgreen!8, rounded corners=1pt},
        dropwin/.style={draw=accent, fill=accent!10, rounded corners=1pt},
        signal/.style={greenish, line width=0.75pt},
        cut/.style={linegray, densely dashed, line width=0.45pt}
      ]
        \foreach \x in {0,1.55,3.10,4.65,5.75} {
          \draw[cut] (\x,0.42) -- (\x,1.42);
        }
        \draw[signal]
          plot[smooth] coordinates {
            (0.00,1.03) (0.18,1.24) (0.36,0.91) (0.55,1.13)
            (0.75,0.83) (0.94,1.28) (1.12,0.99) (1.34,1.17)
            (1.55,1.01) (1.74,1.23) (1.95,0.89) (2.15,1.11)
            (2.36,0.82) (2.56,1.29) (2.78,0.97) (2.96,1.16)
            (3.10,1.02) (3.29,1.22) (3.50,0.90) (3.70,1.14)
            (3.91,0.85) (4.12,1.26) (4.32,0.96) (4.50,1.15)
            (4.65,1.01) (4.84,1.23) (5.05,0.91) (5.24,1.13)
            (5.44,0.88) (5.62,1.08) (5.75,0.98)
          };
        \foreach \x in {0,1.55,3.10} {
          \draw[fullwin] (\x,0.00) rectangle ++(1.55,0.42);
          \node[text=deepgreen] at (\x+0.775,0.21) {150 s};
        }
        \draw[dropwin] (4.65,0.00) rectangle ++(1.10,0.42);
        \node[text=accent!85!black] at (5.20,0.21) {$<150$ s};
        \draw[accent, line width=0.9pt] (4.74,0.07) -- (5.66,0.35);
        \draw[accent, line width=0.9pt] (4.74,0.35) -- (5.66,0.07);
        \draw[-{Latex[length=1.8mm]}, thick, linegray] (-0.05,0.55) -- (5.95,0.55);
      \end{tikzpicture}
    \end{column}
  \end{columns}

  \note{Explicar que esta parte no es secundaria: si el modelo va a combinar datasets, hay que homogeneizar frecuencia, escala, ventanas y sensores ausentes. Remarcar que solo se conservan ventanas completas de 150 segundos; el tramo final se descarta si no llega a esa duración.}
\end{frame}
...
\begin{frame}[c]{Diseño experimental: separar efectos}
  ...
  \begin{tabular}{>{\bfseries}p{0.25\linewidth}p{0.22\linewidth}p{0.26\linewidth}p{0.18\linewidth}}
    \toprule
    Modelo & Inicialización & Preentrenamiento EEG & Fine-tuning \\
    \midrule
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[draw=deepgreen, fill=soft, rounded corners=3pt, inner xsep=8pt, inner ysep=4pt, text width=0.86\linewidth, align=center]{
        \textbf{Lectura:} la transferencia MEG--EEG no resuelve el brain-to-text no invasivo, pero muestra una vía viable y medible para avanzar.
      };
    \end{tikzpicture}
  \end{center}
    ...
    \begin{column}{0.30\linewidth}
      \begin{block}{Trabajo futuro}
        ...
        \begin{itemize}
          ...
          \item Más bandas, remuestreos, tokenizadores y datasets.
...
\begin{frame}
  \begin{columns}[c,totalwidth=\linewidth]
    \begin{column}{0.43\linewidth}
      ...
      \vspace{7mm}
      {\Large\color{white}Preguntas, recomendaciones, incordios\par}
    \end{column}
