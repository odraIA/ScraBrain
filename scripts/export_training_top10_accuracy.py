...
def write_pivot_csv(path: Path, records: Sequence[dict[str, Any]], line_field: str) -> None:
    ...
    for row in records:
        values[(str(row["epoch"]), str(row[line_field]))] = float(row["accuracy"])
    rows = []
    for epoch in epochs:
        out = {"epoch": epoch}
        for line in lines:
...
def summarize_histories(history_files: Sequence[Path], records_by_file: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    summary = []
        ...
        summary.append(
            {
                ...
                "status": "ok" if records else "no_requested_top10_columns",
            }
        )
    return summary
...
def main() -> int:
    ...
    summary_rows = summarize_histories(history_files, records_by_file)
    summary_csv = args.output_dir / "top10_training_curves_manifest.csv"
    write_csv(
        summary_csv,
        summary_rows,
        ["source", "run_id", "model", "stage", "run_dir", "history_file", "top10_records", "status"],
    )
    ...
    if not all_records:
        print("No requested top-10 columns were found. Check manifest CSV for scanned files.")
